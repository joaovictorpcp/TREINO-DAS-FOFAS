...
import { Plus, Trash2, Save, AlertTriangle, BookOpen, HelpCircle, Layers, Link as LinkIcon, Activity, Dumbbell, Bike, Footprints, Waves, ChevronDown, ChevronUp } from 'lucide-react';
...
const ExerciseRow = memo(({
    ...
    const [showDetails, setShowDetails] = useState(
        () => !!(ex.rpe || ex.rir) // Start open if data already exists
    );
    const inputRefs = useRef({});

    // Smooth scroll when focusing a field on mobile
        ...
        <div
            ...
            style={{
                ...
                borderRadius: isSuperset
                    ...
                    : '8px',
                marginBottom: (isSuperset && !isEnd) ? '1px' : (isSuperset ? '0' : '1px'),
        ...
        >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%' }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <input
                            ...
                            type="text"
                            className="input"
                            placeholder={isSuperset ? (isStart ? "Exercício 1 (Conjugado)" : "Exercício + (Conjugado)") : "Nome do exercício..."}
                            value={ex.name}
            ...
            <div className={styles.colNum}>
                ...
                <input
                    ...
                    placeholder="0"
                    value={ex.sets}
                    onChange={(e) => onUpdate(ex.id, 'sets', e.target.value)}
                    onFocus={handleFocus}
            ...
            <div className={styles.colNum}>
                ...
                <input
                    ...
                    type="text"
                    inputMode="decimal"
                    className={clsx("input", ex.suggestProgression && styles.suggestedLoad)}
                    placeholder="kg"
                    value={ex.load}
                    onChange={(e) => onUpdate(ex.id, 'load', e.target.value)}
                    onFocus={handleFocus}
                    onKeyDown={(e) => handleKeyDown(e, 'load')}
                    enterKeyHint="next"
                    autoComplete="off"
                    style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }}
                />
            ...
            {/* RPE - visible on desktop, toggleable on mobile */}
            <div className={styles.colNum}>
                ...
                <input
                    ...
                    placeholder="-"
                    value={ex.rpe}
                    onChange={(e) => onUpdate(ex.id, 'rpe', e.target.value)}
                    onFocus={handleFocus}
            ...
            <div className={styles.colNum}>
                ...
                <input
                    ...
                    placeholder="-"
                    value={ex.rir}
                    onChange={(e) => onUpdate(ex.id, 'rir', e.target.value)}
                    onFocus={handleFocus}
                    enterKeyHint="done"
                    autoComplete="off"
                    style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }}
            ...
            <div className={styles.colAction}>
                <div style={{ display: 'flex', flexDirection: 'row', gap: '8px' }}>
                    <button onClick={() => onRemove(ex.id)} className={styles.iconBtn} title="Remover" style={{ color: 'var(--text-muted)' }}><Trash2 size={18} /></button>
                    <button onClick={() => onAddSuperset(index)} className={styles.iconBtn} style={{ color: 'var(--accent-primary)', borderColor: 'rgba(74, 222, 128, 0.3)' }} title="Conjugar"><Plus size={18} /></button>
                </div>
            </div>
        </div>
    );
...
const TrainingLog = () => {
    ...
    // --- Helpers & Handlers (memoized for ExerciseRow) ---
    const calculateVTT = useCallback((sets, reps, load) => {
    ...
    const isHighStress = useCallback((rpe, rir) => {
    ...
    const addSuperset = useCallback((index) => {
        setExercises(prev => {
            const newArr = [...prev];
    ...
    // --- Build exercise blocks for rendering ---
    ...
    const filledCount = exercises.filter(ex => ex.name.trim() !== '').length;
    ...
    return (
        <div className={clsx("page-container animate-fade-in", isEditing && styles.editingMode)}>
            ...
            {isEditing && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '1rem', background: 'var(--bg-secondary)', borderRadius: '12px', border: '1px solid var(--border-subtle)', marginBottom: '1.5rem' }}>
                    <div style={{
                ...
                <>
                    ...
                    <div className={`glass-panel ${styles.tableContainer}`} style={{ overflow: 'hidden' }}>
                        ...
                        <div className={styles.rowsContainer}>
                            {exerciseBlocks.map((block, bIndex) => {
                                        ...
                                        <div key={`ss-${block.supersetId}-${bIndex}`} className={styles.supersetGroup}>
                                            {block.items.map(({ ex, index }, itemIndex) => (
                                                <ExerciseRow
                                                    ...
                                                    isSuperset={true}
                                                    isStart={itemIndex === 0}
                                                    isEnd={itemIndex === block.items.length - 1}
                                                    onUpdate={updateExercise}
                                                    onRemove={removeRow}
                                                    onAddSuperset={addSuperset}
                                                    calculateVTT={calculateVTT}
                ...
                <div className="glass-panel" style={{ padding: '2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                        ...
                        <div>
                            ...
                            <input type="text" inputMode="numeric" pattern="[0-9]*" className="input" placeholder="ex: 45" value={durationMinutes} onChange={e => setDurationMinutes(e.target.value)} style={{ fontSize: '1.1rem', background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }} />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" inputMode="decimal" className="input" placeholder="ex: 7" value={sessionRpe} onChange={e => setSessionRpe(e.target.value)} style={{ fontSize: '1.1rem', background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }} />
                        </div>

                        <div>
                            <label className="label" style={{ color: 'var(--text-secondary)' }}>Distância ({activityType === 'swimming' ? 'metros' : 'km'})</label>
                            <input type="text" inputMode="decimal" className="input" placeholder={activityType === 'swimming' ? "ex: 1500" : "ex: 5.5"} value={distance} onChange={e => setDistance(e.target.value)} style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }} />
                        </div>

                        <div>
                            <label className="label" style={{ color: 'var(--text-secondary)' }}>Freq. Cardíaca Média (bpm)</label>
                            <input type="text" inputMode="numeric" pattern="[0-9]*" className="input" placeholder="ex: 145" value={avgHr} onChange={e => setAvgHr(e.target.value)} style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }} />
                        </div>
                        ...
                        {activityType === 'cycling' && (
                            <div>
                                ...
                                <input type="text" inputMode="numeric" pattern="[0-9]*" className="input" placeholder="ex: 200" value={avgWatts} onChange={e => setAvgWatts(e.target.value)} style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }} />
                            </div>
                        )}
                        ...
                        <div>
                            ...
                            <input type="text" inputMode="numeric" pattern="[0-9]*" className="input" placeholder="ex: 120" value={elevation} onChange={e => setElevation(e.target.value)} style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }} />
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="glass-panel" style={{ marginTop: '1.5rem', padding: '1.5rem' }}>
                {activityType === 'weightlifting' && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: '1px solid var(--border-subtle)' }}>
                        <div>
                            <label className="label" style={{ color: 'var(--text-secondary)' }}>Duração (minutos)</label>
                            <input type="text" inputMode="numeric" pattern="[0-9]*" className="input" placeholder="ex: 45" value={durationMinutes} onChange={e => setDurationMinutes(e.target.value)} style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }} />
                        </div>
                        <div>
                            <label className="label" style={{ color: 'var(--text-secondary)' }}>RPE da Sessão (0-10)</label>
                            <input type="text" inputMode="decimal" className="input" placeholder="ex: 7" value={sessionRpe} onChange={e => setSessionRpe(e.target.value)} style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }} />
                        </div>
                    </div>
                )}
            ...
            {/* Save Button — sticky on mobile via CSS */}
